...
RESP=$(curl -fsS \
  --max-time 2 \
  -X POST \
  "http://127.0.0.1:${API_PORT}/api/v1/start-call" \
...
echo "$(date -Is) START allowed=${ALLOWED} reason=${REASON} customer=${CUSTOMER} src=${SRC} dst=${DST} callid=${CALLID} rate=${RATE} balance=${BALANCE}" >> "$LOGFILE" 2>/dev/null || true
