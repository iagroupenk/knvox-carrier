...
BEGIN
    ...
    IF v_auth.allowed = true THEN
        INSERT INTO billing.active_calls(call_id, customer_code, src, dst, normalized_dst)
        VALUES (v_call_id, p_customer_code, p_src, p_dst, v_auth.normalized_dst)
...
BEGIN
    ...
    ELSE
        ...
        WHERE enabled = true
          AND v_call.normalized_dst LIKE prefix || '%'
    ...
    IF v_cost > 0 THEN
        ...
        SET prepaid_balance = prepaid_balance - v_cost
        WHERE code = v_call.customer_code
        RETURNING prepaid_balance INTO v_balance;
...
CREATE OR REPLACE FUNCTION billing.cleanup_active_calls(p_max_age_minutes INTEGER DEFAULT 360)
RETURNS INTEGER
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    ...
    WHERE started_at < now() - make_interval(mins => p_max_age_minutes);
